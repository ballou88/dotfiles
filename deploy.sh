...
# Package definitions
ALL_PACKAGES=(aerospace ghostty nvim skhd starship tmux zsh)
LINUX_PACKAGES=(nvim starship tmux zsh)
MACOS_PACKAGES=(aerospace ghostty nvim skhd starship tmux zsh)
COMMON_PACKAGES=(nvim starship tmux zsh)
...
validate_packages() {
    ...
    if [[ ${#invalid_packages[@]} -gt 0 ]]; then
        ...
        print_info "Available packages: ${ALL_PACKAGES[*]}"
        exit 1
    fi
}
...
case $COMMAND in
    list)
        ...
        echo "macOS packages: ${MACOS_PACKAGES[*]}"
        echo
